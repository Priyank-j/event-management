name go-docker-event-optimized.PHONY: build run logs dockerstop run-worker build-worker dockerstopworker
...
logs:
	docker logs $(shell docker ps | grep 'go-docker-event-optimized-${stage}' | awk '{ print $$1 }') -f ;


dockerstop:
	did=$(shell docker ps | grep 'go-docker-event-optimized-${stage}' | awk '{ print $$1 }'); \
	if [ "$$did" ]; \
	...
	docker rm 'go-docker-event-optimized-${stage}'; \
